			...
			if (a > 0 && materials.size()>0) {
				...
				if (materialId != -1) {
					...
					if (!ambientTexturePath.empty())
					{
						...
						currentTexture = LoadTexture(ambientTexturePath, "ambientTexture");
						textures.push_back(currentTexture);
					}
					...
					{
						gps::Texture currentTexture;
						currentTexture = LoadTexture(diffuseTexturePath, "diffuseTexture");
						textures.push_back(currentTexture);
					}
					...
					{
						gps::Texture currentTexture;
						currentTexture = LoadTexture(specularTexturePath, "specularTexture");
						textures.push_back(currentTexture);
					}
				}
			}
